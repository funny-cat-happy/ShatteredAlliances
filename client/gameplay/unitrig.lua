...
function unitrig:init( boardRig, unit )
	...
	if unit:getTraits().magnetic_reinforcement then
		self._mag_resFX = self:createHUDProp("kanim_magnetic_resistance_fx", "effect", "loop", boardRig:getLayer("ceiling"), self._prop )
		self._mag_resFX:setSymbolModulate("innercicrle",0.5,0.8,1,0.1 ) --0.75
		self._mag_resFX:setSymbolModulate("ring_pulse",0.5,0.8,1,0.21)
		self._mag_resFX:setSymbolModulate("inside",0.5,0.8,1,0.05)
		self._mag_resFX:setSymbolModulate("inside2",0.5,0.8,1,0.05)
		if not unit:isKO() or not unit:isDead() then
			self._mag_resFX:setVisible(true)
		else
			self._mag_resFX:setVisible(false)
		end
	end
...
function unitrig:destroy()
	...
	if self._mag_resFX then
		self._boardRig:getLayer("ceiling"):removeProp( self._mag_resFX  )			
	end
...
function unitrig:refreshSpotSound(remove)
	...
	if self._spotSound then
		if not unit or remove or not x or unit:isKO() or unit:getTraits().mainframe_status == "off" or not unit:getSounds().spot then
			self._boardRig:getSounds():stopSound( self._spotSound )
			self._spotSound = nil
		elseif unit:getTraits().monst3r and not unit:getTraits().monster_hacking then  
			self._boardRig:getSounds():stopSound( self._spotSound )
			self._spotSound = nil
		elseif unit:getTraits().data_hacking_spotsound and not unit:getTraits().data_hacking then  
			self._boardRig:getSounds():stopSound( self._spotSound )
			self._spotSound = nil		
			unit:getTraits().data_hacking_spotsound = nil
		else 
...
function unitrig:refreshDatabankFx(transition)
	
	local unit = self:getRawUnit()
	local stage = unit:getTraits().progress

	if not self._databankFx then
		self._databankFx = self:createHUDProp("kanim_databank_fx", "effect", "stage1_in", true,  self:getProp() )--boardrig:getLayer("ceiling")			
	end
	
	self._databankFx:setVisible(false)

	if transition then
		if transition  ~= "off" then
			self._databankFx:setVisible(true)
			if unit:getTraits().progress == 0 then
				self._databankFx:setCurrentAnim("stage1_in")	

				self._databankFx:setListener( KLEIAnim.EVENT_ANIM_END,
					function( anim, animname )
						if animname == "stage1_in" then
							self._databankFx:setCurrentAnim("stage1_loop")	
							--self._databankFx:setPlayMode( KLEIAnim.ONCE )						
						end				
					end )	
			elseif unit:getTraits().progress == 1 then
				self._databankFx:setCurrentAnim("stage_2")	

				self._databankFx:setListener( KLEIAnim.EVENT_ANIM_END,
					function( anim, animname )
						if animname == "stage_2" then
							self._databankFx:setCurrentAnim("stage2_loop")	
							--self._databankFx:setPlayMode( KLEIAnim.ONCE )						
						end
					end )							
			elseif unit:getTraits().progress == 2 then
				self._databankFx:setCurrentAnim("stage3")	

				self._databankFx:setListener( KLEIAnim.EVENT_ANIM_END,
					function( anim, animname )
						if animname == "stage3" then
							self._databankFx:setCurrentAnim("stage3_loop")	
							--self._databankFx:setPlayMode( KLEIAnim.ONCE )						
						end
					end )			
			elseif unit:getTraits().progress == 3 then
				self._databankFx:setCurrentAnim("fail_fadeout")	
				self._databankFx:setListener( KLEIAnim.EVENT_ANIM_END,
					function( anim, animname )
						if animname == "fail_fadeout" then
							self._databankFx:setVisible(false)						
						end
					end )							
			end 
		...
		if unit:getTraits().progress and unit:getTraits().mainframe_status ~= "off" then
			self._databankFx:setVisible(true)
			if unit:getTraits().progress == 0 then
				self._databankFx:setCurrentAnim("stage1_loop")	
			elseif unit:getTraits().progress == 1  then
...
function unitrig:refreshSwitchFx(transition)
	...
	self._switchFx:setVisible(false)

	if transition then
		self._switchFx:setVisible(true)
		if transition == "off" then			
			self._switchFx:setVisible(false)			
		elseif transition == "fail" then			
			...
			self._switchFx:setListener( KLEIAnim.EVENT_ANIM_END,
				function( anim, animname )
					if animname == "stage_"..transition then
						...
						self._switchFx:setListener( KLEIAnim.EVENT_ANIM_END,
							function( anim, animname )
								if animname ==  transition.."_fadeout" then
...
function unitrig:onSimEvent( ev, eventType, eventData )
	...
	if self._state == nil or not self._state:onSimEvent( ev, eventType, eventData ) then
		...
		elseif eventType == simdefs.EV_UNIT_SWTICH_FX then
			local unit = self:getUnit()
			if unit:getTraits().multiLockSwitch then
				self:refreshSwitchFx(eventData.transition)			
			else
				self:refreshDatabankFx(eventData.transition)
			end

		elseif eventType == simdefs.EV_UNIT_HIDE then			
			self:setHidden( eventData.hide )
		end
	end
end
...
function unitrig:refreshProp( refreshLoc )
	...
	if x and y then
		...
		if unit and unit:getTraits().multiLockSwitch then
			local rawunit = self:getRawUnit()
			if rawunit and rawunit:getTraits().switched then		
				if not rawunit:getTraits().unlocked then
					self:refreshSwitchFx("idle_loop")
				else
					self:refreshSwitchFx("pass_loop")
				end
			else
				self:refreshSwitchFx()
			end		
		end
		if unit and unit:getTraits().databank then		
			self:refreshDatabankFx()		
		end
