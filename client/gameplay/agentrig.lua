...
local function doKOanim(self, thread, fx )
	...
	if not unit:getTraits().ontheground then
...
local function getIdleAnim( sim, unit )
	...
	elseif unit:getTraits().monster_hacking or unit:getTraits().data_hacking  then 
		return "tinker_loop"
...
function idle_state:onEnter()
	...
	if unit:isGhost() then
		...
		if idleAnim == "overwatch" and (unit:isNPC() or unit:getTraits().disguiseOn) then
	...
	elseif eventType == simdefs.EV_UNIT_SHOT then
		...
		for shotNum = 1, eventData.dmgt.shots do
			...
			if not sounds then				
				...
				if eventData.dmgt.extraShoot then
					self._rig:playSound( eventData.dmgt.extraShoot )
				end
			end
...
function agentrig:refreshAnim( unit )
	...
	elseif not weapon then
		...
		elseif rawUnit and rawUnit:getPlayerOwner() and rawUnit:getPlayerOwner():isNPC() then
