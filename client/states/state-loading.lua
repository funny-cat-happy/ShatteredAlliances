...
local function runLoadThread( stateLoad, params, stateGame, campaign, simHistoryIdx )
    ...
    if not auto_load then
        ...
        if campaign and not endless_mode then
            if campaign.incognitaLevel == 1 then
            ...
            elseif campaign.incognitaLevel == 2 then
...
local function regenerateLevel( self, campaign )
...
local function onLoadCampaignError( campaign, params, self, result )
    ...
    if version.isVersionOrHigher( params.missionVersion or "", "0.17.7" ) and result and result:find( "BAD_LEVEL" ) ~= nil then
        -- silently try again, with new seed
        regenerateLevel( self )
    else
	    util.coDelegate(
		    function()
