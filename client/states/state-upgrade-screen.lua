...
local function changeStashIndex(self, delta)
	self._stashIndex =  self._stashIndex + delta
...
upgradeScreen.refreshInventory = function( self, unitDef, index )
	...
	for i, widget in self.screen.binder:forEach( "agency_inv_" ) do
		...
		if self._agency.upgrades and  self._agency.upgrades[item] then
			local itemDef, upgradeParams
            if type(self._agency.upgrades[item]) == "string" then
                itemDef = unitdefs.lookupTemplate( self._agency.upgrades[item] )
            else
                upgradeParams = self._agency.upgrades[item].upgradeParams
                itemDef = unitdefs.lookupTemplate( self._agency.upgrades[item].upgradeName )
            end
			local itemUnit = simfactory.createUnit( util.extend( itemDef )( upgradeParams and util.tcopy( upgradeParams )), nil )
            ...
            widget.binder.btn.onDragStart = util.makeDelegate( self, "onDragStorage", self._agency.upgrades[item], itemUnit, widget.binder.btn.onClick )

		else
            guiex.updateButtonEmptySlot( widget )
		end
	end	
	if self._stashIndex > 0 then
		self.screen:findWidget("cycleLeftBtn"):setVisible(true)
	else
		self.screen:findWidget("cycleLeftBtn"):setVisible(false)
	end

	if self._agency.upgrades and self._stashIndex + 4 < #self._agency.upgrades then
		self.screen:findWidget("cycleRightBtn"):setVisible(true)
	else
		self.screen:findWidget("cycleRightBtn"):setVisible(false)
	end
end
...
mui.activateScreen( self.screen )
	...
	self._stashIndex = 0
	self.screen:findWidget("cycleLeftBtn"):setVisible(false)
	self.screen:findWidget("cycleLeftBtn").onClick = util.makeDelegate( nil, changeStashIndex, self, -1)
	self.screen:findWidget("cycleRightBtn"):setVisible(false)
	self.screen:findWidget("cycleRightBtn").onClick = util.makeDelegate( nil, changeStashIndex, self, 1)
