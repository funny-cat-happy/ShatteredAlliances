...
local function onClickShowLogs( self, agentDef )

	MOAIFmodDesigner.playSound( "SpySociety/HUD/menu/popup" )

    local dialog = include( "fe/agent-logs" )
    dialog.onClose = function()
    end
    dialog:show( agentDef )
end


local function setFooter(name,icon, footer)
	name:setVisible(true)
	icon:setVisible(true)
	name:setText(footer[1])
	icon:setImage(footer[2])

	print(STRINGS.UI.DLC_TAG, footer[1],footer[2],footer[3])

	local author = STRINGS.UI.DLC_TAG_TYPE_MOD
	if footer[3] == "Klei" then
		author = STRINGS.UI.DLC_TAG_TYPE_EXPANSION
	end


	name:setTooltip(util.sformat(STRINGS.UI.DLC_TAG,footer[1],author))
	icon:setTooltip(util.sformat(STRINGS.UI.DLC_TAG,footer[1],author))	
end
...
local function selectProgram( self, programIdx, programID )
	...
	programPanel.binder["dlcName"]:setVisible(false)
	programPanel.binder["dlcIcon"]:setVisible(false)

	if programData.dlcFooter then
		setFooter( programPanel.binder["dlcName"], programPanel.binder["dlcIcon"], programData.dlcFooter)
	end
...
local function selectAgent( self, agentIdx, agentID, transition, noVoice )
	...
	agentWidget.binder.dlcIcon:setVisible(false)
	agentWidget.binder.dlcName:setVisible(false)

	if agentDef.traits.dlcFooter then
		setFooter( agentWidget.binder.dlcName, agentWidget.binder.dlcIcon, agentDef.traits.dlcFooter)
	end
	...
	agentWidget.binder.logBtn:setVisible(false)
	...
	if metadefs.isRewardUnlocked( agentID ) then
		...
		if not noVoice and not self.mute then
			stopVoice(self)
		...
		if agentDef.logs and #agentDef.logs >0 then

			agentWidget.binder.logBtn:setVisible(true)
			agentWidget.binder.logBtn.onClick =  util.makeDelegate( nil, onClickShowLogs, self, agentDef )
		end
	else
...
local function muteToggle(self)
	local settings = savefiles.getSettings( "settings" )

	if not self.mute then
		self.mute = true
		stopVoice(self)
		self._panel.binder.muteTxt:setVisible(true)		
		settings.data.muteCentralTeamSelect = true
		settings:save()
	else
		self.mute = nil
		self._panel.binder.muteTxt:setVisible(false)
		settings.data.muteCentralTeamSelect = nil
		settings:save()
	end
end
...
function teamPreview:onLoad()
	...
	for i,program in ipairs(serverdefs.SELECTABLE_PROGRAMS[1]) do 
		...
		if programData.abilityOverride then
			serverdefs.SELECTABLE_PROGRAMS[1][i] = programData.abilityOverride[1]
		end
	end
	...
	for i,program in ipairs(serverdefs.SELECTABLE_PROGRAMS[2]) do 
		...
		if programData.abilityOverride then
			serverdefs.SELECTABLE_PROGRAMS[2][i] = programData.abilityOverride[1]
		end
	end
	...
	self._panel.binder.muteBtn.onClick = util.makeDelegate( nil, muteToggle, self )
	self._panel.binder.muteTxt:setVisible(false)

	local settings = savefiles.getSettings( "settings" )
	if settings.data.muteCentralTeamSelect then
		 muteToggle(self)
	end
	...
	if #serverdefs.SELECTABLE_AGENTS < 14 then
		local bump = (14 - #serverdefs.SELECTABLE_AGENTS) * 54
		local x1,y1 = lb:getPosition()
		lb:setPosition(x1 + bump, y1)

		local title =self.screen:findWidget( "available_txt" )
		local x2,y2 = title:getPosition()
		title:setPosition(x2 + bump, y2)
	end
