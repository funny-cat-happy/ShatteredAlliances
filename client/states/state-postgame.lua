             ...
             --   local hoursLeft = campaign.difficultyOptions.maxHours - campaign.hours
                if event.eventType == simdefs.CAMPAIGN_EVENTS.ADD_MORE_HOURS then
                    if self._params.situationName == event.mission then
                        ...
                        if not campaign.extendedCampaignHours then
                            campaign.extendedCampaignHours = 0
                        end
                        campaign.extendedCampaignHours = campaign.extendedCampaignHours + event.data                        
                    end
                end
                    ...
                    if self._params.situationName == event.mission then
                        if event.data.agency then
                          ...
                          --  campaign.missionEvents[event.data.params] = true
                            if event.data.editCampaign then
                                print("EDIT THE CAMPAIGN",event.data.param,"=",event.data.value)
                                campaign[event.data.param] = event.data.value
                            else
                                if event.data.value then
                ...
                --removes custom scripts after the mission is complete.                 
            ...
            function()
                ...
                rewardsDialog:show( false, function( option )
                    ...
                    elseif option == 2 then
                        ...
                        if campaign.extendedCampaignHours then
                            olddifficultyOptions.maxHours = olddifficultyOptions.maxHours - campaign.extendedCampaignHours 
                        end
    ...
    elseif flow_result == ENDGAMEFLOW.CONTINUE_CAMPAIGN then
        ...
        if campaign.campaignEvents then
            for i=#campaign.campaignEvents,0,-1 do
                ...
                -- CHECK FOR CAMPAIGN EVENTS THAT ADD MAP POPUPS
