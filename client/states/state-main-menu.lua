...
local function checkModsLoaded (self)
	local settingsFile = savefiles.getSettings( "settings" )
	
	
	if settingsFile.data and settingsFile.data.mods_updated then
		self.screen.binder.noticeTxt:spoolText( "<c:01DE02>".. util.sformat(STRINGS.UI.MODS_UPDATED).."</>" )
		settingsFile.data.mods_updated = nil	
		settingsFile:save()	
	end
end
...
function mainMenu:onLoad()
	...
	self.screen.binder.noticeTxt:setText("")
