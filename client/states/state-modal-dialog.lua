...
local function createUpdateDisclaimerDialog_b( okStr, readMoreStr, resetStr )
	local t = util.tcopy( _M )
	...
	t.onLoad = function( self )
		...
		self.screen.binder.resetBtn.binder.btn:setText(resetStr)
		self.screen.binder.resetBtn.binder.btn.onClick = util.makeDelegate( nil, function() t.result = AUX2 end )		
	end
...
local function showUpdateDisclaimer_b( okTxt, readMoreTxt, regenTxt )
	MOAIFmodDesigner.playSound(  cdefs.SOUND_HUD_MENU_POPUP )
	local modalDialog = createUpdateDisclaimerDialog_b( okTxt, readMoreTxt, regenTxt )
	return showDialog( modalDialog )
end

local function showDataCollection( body, title, buttton1, button2 )
	MOAIFmodDesigner.playSound(  cdefs.SOUND_HUD_MENU_POPUP )
	local modalDialog = createModalDialog(  body, title, true, buttton1, button2 )
	return showDialog( modalDialog )
end
