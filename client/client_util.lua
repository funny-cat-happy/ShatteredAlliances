...
function tooltip_section:addFooter( txt, img )
	local widget = self._tooltip._screen:createFromSkin( "tooltip_section_dlc" )
	widget.binder.txt:setText( txt )
	widget.binder.img:setImage( img )
	self._widget:addChild( widget )
	widget.activate = function( self, screen )
		local TOP_BUFFER = 4
		local W, H = screen:getResolution()
		...
		local th = math.ceil(H * (ymax - ymin) / 2) * 2 + TOP_BUFFER
		widget.binder.txt:setSize( nil, th )
		widget.binder.txt:setPosition( nil, (th / -2) - TOP_BUFFER )
		widget.binder.img:setPosition( nil, (th / -2) - TOP_BUFFER + 2  )
	end
