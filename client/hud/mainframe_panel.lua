...
local function createActivateTooltip( hud, unit, useData )
	...
	if unit:getTraits().range then
		...
		tooltip.activate = function ( )
			...
			local coordsTest = simquery.rasterCircle( hud._game.simCore, x0, y0, unit:getTraits().range )
			local coords ={}

			if unit:getTraits().attackNeedsLOS then
				for i=1,#coordsTest-1,2 do
					local cell = unit:getSim():getCell(coordsTest[i],coordsTest[i+1])
					if cell then
						local raycastX, raycastY = unit:getSim():getLOS():raycast(x0, y0, cell.x, cell.y)
						if raycastX == cell.x and raycastY == cell.y then
							table.insert(coords, coordsTest[i])
							table.insert(coords, coordsTest[i+1])						
						end			
					end
				end
			end
