...
function mission_panel:processEvent( event )
	...
	elseif event.type =="newLocationName" then
		self._hud._game.simCore:getTags().newLocationName = event.name
		self._hud:refreshHud()
	elseif event.type =="showMissionObjectives" then
	    ...
	    local objectives = situationData.ui.objectives

	    -- scan the missionEvents for objectives that need to be in the main list.
	    if self._hud._game.params.missionEvents then
		    for i,event in pairs(self._hud._game.params.missionEvents) do	 

		    	--cxt.params.missionEvents.needPowerCells
		    	local req = true

		    	if type(event) == "table" and event.req then
		    		local sim = self._hud._game.simCore
		    		if not sim:hasTag(event.req) then
		    			req = false
		    		end
		    	end

		    	if type(event) == "table" and event.type and event.type == "objective" and req then
		    		objectives = objectives .."\n"..event.text
		    	end
		    end
		end

		modalDialog.showObjectivesDialog( missionTxt, objectives, situationData.ui.secondary_objectives, situationData.ui.insetImg, corpData.imgs.logoLarge )
	...
	elseif event.type == "modalConversation" then
		if event.script then 
			...
			local modaltalkinghead = talkinghead( storyheadscreen, storyheadscreen.binder.Friends, true, self._hud._game)
