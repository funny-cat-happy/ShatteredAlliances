...
local function MakeTestData()
    local test_data =
    {
        ...
        devices = {
            ...
            {type="drone_akuma",hacked=true,seen=true},
        },
    }
...
function mission_recap_screen:init()
    ...
    for i,device in ipairs (mod_manager.device_types) do
        table.insert(device_types,device)
    end
    for i,guard in ipairs (mod_manager.guard_types) do
        table.insert(guard_types,guard)
    end
    for i,source in ipairs (mod_manager.credit_sources) do
        table.insert(credit_sources,source)
    end
    for i,sink in ipairs (mod_manager.credit_sinks) do
        table.insert(credit_sinks,sink)
    end
    for i,device in pairs (mod_manager.device_lookup) do        
        device_lookup[i] = device        
    end
    for i,guard in pairs (mod_manager.guard_lookup) do
        guard_lookup[i] = guard
    end    


    self.screen = mui.createScreen( "mission_recap_screen.lua" )

    self.screen:findWidget("pnl.logBtn"):setVisible(false)
    self.screen:findWidget("pnl.logBtn.btn"):setText(STRINGS.UI.NEW_LOG)
...
function mission_recap_screen:openLog(newLog)
                       
        self:PlaySound( "SpySociety/HUD/menu/popup" )
...
function mission_recap_screen:ShowAgentData()

    if  self.mission_data.newLogData then
                       
        local widget = self.screen:findWidget("pnl.logBtn")
        widget.binder.btn.onClick = function() self:openLog(self.mission_data.newLogData) end
