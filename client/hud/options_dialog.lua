...
local function retrieveSettings( dialog )
    ...
    settings.schedule_mod_update = dialog._schedule_mod_update
...
local function onClickAccept( dialog )
    ...
    if not needsConfirm or modalDialog.showYesNo( STRINGS.OPTIONS.CONFIRM, STRINGS.OPTIONS.CONFIRM_TITLE ) == modalDialog.OK then
        ...
        local privacySettingsFile = savefiles.getPrivacySettings()
        privacySettingsFile.data.suppressMetricsData = settings.suppressMetricsData
        privacySettingsFile:save()
...
local function onClickMetricsBtn( dialog, widget )
    if widget._checked == 1 then

        local result =  modalDialog.showDataCollection(STRINGS.OPTIONS.DATA_COLLECTION_BODY, STRINGS.OPTIONS.DATA_COLLECTION_TITLE, STRINGS.OPTIONS.DATA_COLLECTION_OK, STRINGS.OPTIONS.DATA_COLLECTION_VISIT )

        if result == 0 then
            MOAISim.visitURL( config.PRIVACYPORTAL_URL )
            return
        end
...
local function onClickRefreshMods( dialog )
    if mod_manager:hasContentMods() then
        log:write( "Content mods are loaded. Scheduling update for next restart." )
        modalDialog.show( STRINGS.UI.MODS_REFRESH_SCHEDULED )
        dialog._schedule_mod_update = true
    else
        log:write( "No content mods loaded." )
...
function options_dialog:init(game)
    ...
    screen.binder.metricsBtn.onClickCoDelegate = util.makeDelegate( nil, onClickMetricsBtn, self )
    --
