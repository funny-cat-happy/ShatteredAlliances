...
function hud:showFlyText(x0, y0, txt, color, target, sound, soundDelay, delay)
	...
	local fxmgr = self._game.fxmgr
	if delay then
		local delayThread = MOAICoroutine.new()
		delayThread:run(function()
			local delay = delay
			local loop = true
			while loop do
				if delay > 1 then
...
function hud:showFloatText(x0, y0, txt, color)
	local widget = self._world_hud:createWidget(world_hud.HUD_FLOATERS, "flying_text",
		{ worldx = x0, worldy = y0, worldz = 32, t = 0 }, updateFloatTxt)
...
local function doMoveUnit(hud, unit, cellx, celly)
	...
	if sim:getTags().blockMoveObserve then
		hud:showWarning(STRINGS.UI.WARNING_OBSERVE_GUARD, { r = 1, g = 1, b = 1, a = 1 },
			STRINGS.UI.WARNING_OBSERVE_GUARD_2)
	...
	if sim:getTags().blockMoveCentral then
		if not unit:getTraits().central then
			...
			if sim:getQuery().cellHasTag(sim, endcell, "interruptNonCentral") then
				...
				hud:showWarning(STRINGS.UI.WARNING_NO_CENTRAL, { r = 1, g = 1, b = 1, a = 1 },
					STRINGS.UI.WARNING_NO_CENTRAL_2)
				MOAIFmodDesigner.playSound("SpySociety/HUD/gameplay/HUD_Monster_ForSale")
				return false
			end
		end
...
function hud:setShortWalls(do_short)
	...
	self._screen.binder.topPnl.binder.btnToggleWalls:setInactiveImage(do_short and
	"gui/hud3/UserButtons/userbtn_raise_walls.png" or "gui/hud3/UserButtons/userbtn_lower_walls.png")
	self._screen.binder.topPnl.binder.btnToggleWalls:setActiveImage(do_short and
	"gui/hud3/UserButtons/userbtn_raise_walls_hl.png" or "gui/hud3/UserButtons/userbtn_lower_wall_hl.png")
	self._screen.binder.topPnl.binder.btnToggleWalls:setHoverImage(do_short and
	"gui/hud3/UserButtons/userbtn_raise_walls_hl.png" or "gui/hud3/UserButtons/userbtn_lower_wall_hl.png")
	...
	if not ability.acquireTargets then
		...
		if not targetHandler:hasTargets() then
			...
			if defaultTarget and not ability.noDefaultTarget then
				...
				transition(self, STATE_ABILITY_TARGET,
					{ abilityOwner = abilityOwner, abilityUser = abilityUser, ability = ability, targetHandler =
					targetHandler })
			end
		end
	end
...
function hud:transitionItemTarget(item, itemUser)
	local isSameItem = (self._state == STATE_ABILITY_TARGET or self._state == STATE_ITEM_TARGET) and
	item == self._stateData.item
	transition(self, STATE_NULL)
...
local function updateIngameCursor(self, sim, localPlayer)
	if not self._hideCubeCursor and self._tooltipX then
		if not self._game.fxmgr:containsFx(self.selectFX) then
			self.selectFX = self._game.fxmgr:addAnimFx({ x = 0, y = 0, kanim = "gui/selectioncubetest", symbol =
			"character", anim = "anim", loop = true, scale = 1.0 })
		end
...
function hud:transferDaemonProgram()
	...
	if self._daemonCenterPanelUp.ability.duration or self._daemonCenterPanelUp.ability.ice then
		self._mainframe_panel:addMainframeProgram(player, self._daemonCenterPanelUp.ability,
			self._daemonCenterPanelUp.idx)
		move = true
	end
...
function hud:onInputEvent(event)
	...
	if self._state == STATE_ABILITY_TARGET then
		...
		elseif self._stateData.targetHandler then
			...
			if target then
				agent_actions.performAbility(self._game, self._stateData.abilityOwner, self._stateData.abilityUser,
					self._stateData.ability, target)
				return true
			end
		end
	elseif self._state == STATE_ITEM_TARGET then
		...
		elseif self._stateData.targetHandler and self._stateData.targetHandler.onInputEvent then
			local target = self._stateData.targetHandler:onInputEvent(event)
			if target then
				agent_actions.performAbility(self._game, self._stateData.item, self._stateData.itemUser,
					self._stateData.ability, target)
				return true
			end
		end
