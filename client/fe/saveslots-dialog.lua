...
local function onClickContinue( dialog )
	...
	if campaign.missionVersion and  not version.isVersionOrHigher( campaign.missionVersion or "", version.VERSION ) then
		local result = modalDialog.showUpdateDisclaimer_b( STRINGS.UI.SAVESLOTS.CONTINUE, STRINGS.UI.SAVESLOTS.SEE_PATCHNOTES, STRINGS.UI.SAVESLOTS.REGEN )
		if result == modalDialog.AUX then
			MOAISim.visitURL( config.PATCHNOTES_URL )
			return
		end
		if result == modalDialog.AUX2 then
			...
			for i, portrait in widget.binder:forEach("img") do
				...
				if unitDef then
                    ...
                    if template then
                    	...
                    	if canContinue then
                    		-- this is a hack. in the event that you have a DLC saved game that you no longer
                    		-- own, but we'll let you keep playing if it's still installed.
	    					portrait:setImage( "gui/profile_icons/dlc_agent_64x64.png" )
                    	else
