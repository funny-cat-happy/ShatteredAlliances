...
local function onClickBuyFromMonst3r(self, itemData, value )
	...
	if self._agency.cash >= value then
...
local function onClickSellToMonst3r(self,itemData,upgradeIndex,rate)
	...
	if itemData.value then
    	sellValue = math.ceil(itemData.value * rate)
    end
...
function dialog:showMonst3rItem( templateName )
    ...
    local value = 0
    if itemData.value then
    	value = math.floor( itemData.value * MONST3R_DISCOUNT / 10 ) * 10
	end
...
function dialog:refreshAgencyUpgrades()
	...
	for i, widget in sellPanel.binder:forEach("item") do
		...
		if self._agency.upgrades and self._agency.upgrades[item] then
			...
			self:refreshAgencyUpgrade( self._agency.upgrades[item],widget,item)
		else
	...
	if self._stashIndex > 0 then 
		sellPanel.binder.cycleLeftMonsterBtn:setVisible(true)
	else
		sellPanel.binder.cycleLeftMonsterBtn:setVisible(false)
	end		

	if self._agency.upgrades and self._stashIndex + 8 < #self._agency.upgrades then
		sellPanel.binder.cycleRightMonsterBtn:setVisible(true)
	else
		sellPanel.binder.cycleRightMonsterBtn:setVisible(false)
	end	
...
function dialog:refreshAgencyUpgrade(upgrade,widget,upgradeIndex)
	...
	widget.binder.cost:setText( string.format( "+%d", math.ceil(value * 0.5)))
...
function dialog:show( campaign, agency )
    ...
    self._stashIndex = 0
    local sellPanel = screen:findWidget("sell")
	sellPanel.binder.cycleLeftMonsterBtn:setVisible(false)
	sellPanel.binder.cycleLeftMonsterBtn.onClick = util.makeDelegate( nil, changeStashIndex, self, -1)
	sellPanel.binder.cycleRightMonsterBtn:setVisible(false)
	sellPanel.binder.cycleRightMonsterBtn.onClick = util.makeDelegate( nil, changeStashIndex, self, 1)
