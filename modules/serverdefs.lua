...
local MAX_DIFFICULTY_DEFAULT = {MAX=10}
local MAX_DIFFICULTY = {MAX=10}


local ENDLESS_ALERT_DAYS_DEFAULT = {DAY=4}
local ENDLESS_ALERT_DAYS = {DAY=4}
...
local function getDifficulty( campaign, gen )
    ...
    return math.min( MAX_DIFFICULTY.MAX, difficulty )
end
...
local function createCustomFinalSituation( campaign, customFinalSituation )
	...
	if not campaign.lastMissionIndex then
		campaign.lastMissionIndex = 0
	end

	campaign.lastMissionIndex = campaign.lastMissionIndex +1 
...
local function advanceCampaignTime( campaign, hours )
	...
	local previousDay = campaign.currentDay
    ...
    if previousDay ~= campaign.currentDay then
    	campaign.incognitaLevel = campaign.incognitaLevel + 1
    end
	...
	elseif campaign.currentDay > campaign.previousDay then
        ...
        local MAX_DIFFICULTY_LOC = (campaign.difficultyOptions.maxHours == math.huge and MAX_DIFFICULTY.MAX or 4)

		for i,situation in ipairs(campaign.situations) do
            situation.difficulty = math.min( MAX_DIFFICULTY_LOC, situation.difficulty + 1 )
		end 
		...
		if campaign.difficultyOptions.maxHours == math.huge and campaign.currentDay >= ENDLESS_ALERT_DAYS.DAY then 
	...
	if campaign.campaignEvents then
        for i=#campaign.campaignEvents,0,-1 do
        	...
        	if event then
        		if event.eventType == simdefs.CAMPAIGN_EVENTS.ADD_ENDLESS_MISSION then
        			if campaign.difficultyOptions.maxHours == math.huge then
        				if campaign.hours >= (event.day-1)*24 then
		        			
							local rand = include( "modules/rand" )
							local gen = rand.createGenerator( campaign.seed )

							local difficulty = 2 + math.floor( campaign.hours  / 24)

						    local situation = createNewSituation( campaign, gen, util.tmerge( { event.mission }, ALL_LOCATION_TAGS ), difficulty )						    
							campaign.situations = {situation}
    ...
    if campaign.version == version.VERSION then
        ...
        for setting, value in pairs( simdefs.DIFFICULTY_OPTIONS[ simdefs.NORMAL_DIFFICULTY ] ) do
            if campaign.difficultyOptions[ setting ] == nil then
                campaign.difficultyOptions[ setting ] = valuecreateNewSituation
                log:write( "\tAssigning difficulty option: %s = %s", tostring(setting), tostring(value) )
            end
        end
    end

    campaign.incognitaLevel = campaign.incognitaLevel or campaign.currentDay 
...
function ResetSituations()
	...
	util.tclear(MAX_DIFFICULTY)
	util.tmerge(MAX_DIFFICULTY, MAX_DIFFICULTY_DEFAULT)

	util.tclear(ENDLESS_ALERT_DAYS)
	util.tmerge(ENDLESS_ALERT_DAYS, ENDLESS_ALERT_DAYS_DEFAULT)	
end

function ResetLogs()
	log:write("ResetLogs()")
	util.tclear(LOGS)
	util.tmerge(LOGS, DEFAULT_LOGS)
end
...
return
{
	...
	ENDLESS_ALERT_DAYS = ENDLESS_ALERT_DAYS,
