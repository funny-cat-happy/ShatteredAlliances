...
function Actions.MarkInterestInvestigated(sim, unit)
    ...
    if #units > 0 then
        ...
        if destroyedUnit then
            ...
            sim:dispatchEvent(simdefs.EV_PLAY_SOUND,
                { sound = "SpySociety/HitResponse/hitby_tazer_flesh", x = x1, y = y1 })
        end
    end
...
local function checkLookaround(sim, unit, delta)
    ...
    if sim:isVersion("0.17.9") then
        sim:dispatchEvent(simdefs.EV_PULSE_SCAN, {
            unit = unit,
            cells = probeCells,
            range = unit:getTraits()
                .lookaroundRange
        })
    end
