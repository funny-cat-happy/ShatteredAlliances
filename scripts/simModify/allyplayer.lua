...
local allyplayer = class(simplayer)

function allyplayer:init(sim)
        ...
        if unit and unit:isValid() then
            ...
            if newPatrol then
                ...
                if unit:getTraits().patrolPath and #unit:getTraits().patrolPath > 1 then
                    ...
                    unit:getBrain():getSenses():addInterest(firstPoint.x, firstPoint.y, simdefs.SENSE_RADIO,
                        simdefs.REASON_PATROLCHANGED, unit)
                end
            else
...
function allyplayer:updateTracker(sim)
...
function allyplayer:huntAgents(count)
    local hunters = {}
    for _, agent in ipairs(self._sim:getPC():getUnits()) do
        if count > 0 and simquery.isAgent(agent) and not agent:isDown() and not agent:getTraits().takenDrone and not self:isAgentHunted(agent) then
            ...
            if closestHunter then
                ...
                agent:getSim():dispatchEvent(simdefs.EV_SHOW_DIALOG,
                    { dialog = "locationDetectedDialog", dialogParams = { agent } })
...
function allyplayer:isAgentHunted(unit)
...
function allyplayer:spawnInterest(x1, y1, sense, reason, sourceUnit)
...
function allyplayer:spawnInterestWithReturn(x1, y1, sense, reason, sourceUnit, guardsNotToCheck)
    local guardTable = {}
    for i, unit in pairs(self:getUnits()) do
        ...
        -- dont select CFOs or Scientists as targets for the interest point.
        print(unit:getName(), unit:getSim():isVersion("0.17.9"), unit:getTraits().pacifist)
        if unit:getSim():isVersion("0.17.9") and unit:getTraits().pacifist then
            found = true
        end
...
function allyplayer:isNPC()
    return true
end

function allyplayer:isPC()
    return false
end
...
function allyplayer:updateSenses(unit)
...
function allyplayer:tickBrain(unit)
    local thought = nil
...
function allyplayer:onUnitAdded(unit)
    simplayer.onUnitAdded(self, unit)
    self:prioritiseUnits()
end

function allyplayer:onUnitRemoved(unit)
    simplayer.onUnitRemoved(self, unit)
    self:prioritiseUnits()
end

function allyplayer:tickAllBrains()
...
function allyplayer:prioritiseUnits()
...
function allyplayer:getPrioritisedUnits()
    return self.prioritisedUnits
end

function allyplayer:thinkHard(sim)
    local st = os.clock()
    local steps = 0
...
function allyplayer:getIdleSituation()
    return self.situations[simdefs.SITUATION_IDLE]
end

function allyplayer:returnToIdleSituation(unit)
...
function allyplayer:findExistingInvestigateSituation(interest)
...
function allyplayer:createOrJoinInvestigateSituation(unit, interest)
...
function allyplayer:findExistingCombatSituation(target)
...
function allyplayer:findExistingCombatSituationInRange(unit, range)
...
function allyplayer:createOrJoinCombatSituation(unit, target)
...
function allyplayer:findExistingHuntSituation(targetCell)
...
function allyplayer:createOrJoinHuntSituation(unit, target)
...
function allyplayer:findExistingFleeSituation(unit)
...
function allyplayer:createOrJoinFleeSituation(unit)
...
function allyplayer:joinEnforcerHuntSituation(unit, target)
...
function allyplayer:processReactions(unit)
...
function allyplayer:onEndTurn(sim)
    simplayer.onEndTurn(self, sim)
end

function allyplayer:onTrigger(sim, evType, evData)
