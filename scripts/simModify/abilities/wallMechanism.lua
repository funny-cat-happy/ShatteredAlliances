...
local wallMechanism =
{
    ...
    acquireTargets = function(self, targets, game, sim, abilityOwner, abilityUser)
        ...
        if type(applyFn) == "string" then
            applyFn = simquery
                [applyFn] -- Sooo brutal, but due to circular includes, itemdefs cannot directly reference simquery functions
        end

        for dir in pairs(fromCell.sides) do
            if applyFn(sim, fromCell, dir) then
                table.insert(walls, { cell = { x = fromCell.x, y = fromCell.y }, dir = dir })
            end
        end

        return targets.wallTarget(game, walls, self, abilityOwner, abilityUser)
    end,
    ...
    executeAbility = function(self, sim, unit, userUnit, x0, y0, dir)
        ...
        local wallDevice = SAInclude(string.format("simModify/units/%s", unit:getTraits().wallDevice))
        wallDevice.applyToWall(sim, fromCell, dir, unit, userUnit)
