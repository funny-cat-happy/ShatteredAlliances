			    ...
			    if simquery.isEnemyAgent(player, cellUnit, ignoreDisguise) then
					if unit then					
						...
						if unit:getTraits().relayInterest then
							...
							if guard then		
							   	guard:getBrain():getSenses():addInterest( cell.x, cell.y, simdefs.SENSE_SIGHT, simdefs.REASON_SCANNED, cellUnit, ignoreDisguise)
							end
							self:processReactions( unit )
						else
			        		unit:getBrain():getSenses():addInterest( cell.x, cell.y, simdefs.SENSE_SIGHT, simdefs.REASON_SCANNED, cellUnit, ignoreDisguise)
			        	end
			        else
					    self:triggerEvent( simdefs.TRG_NEW_INTEREST, { x = x0, y = y0, range = simdefs.SOUND_RANGE_3, interest = { x= cell.x, y = cell.y, reason=simdefs.REASON_SCANNED} })
				    end
			    end
