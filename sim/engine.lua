...
local router = include("sim/units/router")
	...
	if coords then
		for i = 1, #coords, 2 do
			...
			if coord_cell then
				-- Apply impass
				if unit:getTraits().impass then
					assert(coord_cell.impass + 1 * apply >= 0,
						string.format("Illegal impass: %d", coord_cell.impass + 1 * apply))
...
function simengine:init(params, levelData)
	...
	self._icePrograms = self:handleOverrideAbility(util.tcopy(levelOutput.ice_programs or {}))
		...
		if player and player:isNPC() then
			...
			if unit:getBrain() and player:getIdleSituation() then
				...
				elseif unit:getTraits().reconPatrol then
					...
					if self:nextRand(1, 3) > 1 or unit:getTraits().always_patrol then
...
function simengine:handleOverrideAbility(list)
	--filter out the outdated version of programs
	for i, ability in ipairs(list) do
		if type(ability) == "string" then
			local abilityDef = abilitydefs.lookupAbility(ability)

			local loop = true
			while abilityDef and loop do
				if abilityDef.overrideAbility and self:isVersion(abilityDef.overrideAbility.version) then
					if self:isVersion(abilityDef.overrideAbility.version) then
						local override = abilityDef.overrideAbility.override

						abilityDef = abilitydefs.lookupAbility(override)
						if abilityDef then
							list[i] = override
						else
							loop = false
...
function simengine:refreshUnitLOS(unit)
	...
	if #oldcells > 0 or #newcells > 0 then
		...
		self:triggerEvent(simdefs.TRG_LOS_REFRESH, { seer = unit, cells = updateCells })
	end
...
function simengine:warpUnit(unit, cell, facing, reverse)
	...
	self:dispatchEvent(simdefs.EV_UNIT_WARPED,
		{ unit = unit, from_cell = oldcell, to_cell = cell, facing = unit:getFacing(), reverse = reverse })
	...
	self:getTags().lostTargetSpeech = nil
end
...
function simengine:modifyExit(cell, dir, exitOp, unit, stealth)
	...
	elseif exitOp == simdefs.EXIT_DISARM then
		...
		self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = "ALARM DISABLED", x = x1, y = y1, color = { r = 163 / 255, g = 0, b = 0, a = 1 }, alwaysShow = true })
	end
	...
	if exitOp == simdefs.EXITOP_BREAK_DOOR then
		...
		for i, targetUnit in ipairs(altCell.units) do
			if simquery.isAgent(targetUnit) and not targetUnit:isKO() and not targetUnit:isDead() and targetUnit:getArmor() == 0 then
				self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = STRINGS.UI.FLY_TXT.DOOR_SLAMMED, x = altCell.x, y = altCell.y, color = { r = 163 / 255, g = 0, b = 0, a = 1 }, alwaysShow = true })
				local koCount = 1
...
function simengine:tryShootAt(sourceUnit, targetUnit, dmgt0, equipped)
	...
	sourceUnit:setDisguise(false, nil, true)
	...
	if targetUnit:isValid() and targetUnit:getPlayerOwner() then
		if not self:isVersion("0.17.9") or not equipped:getTraits().doNotInterruptMove then
...
function simengine:damageUnit(targetUnit, srcDamage, kodamage, fx, sourceUnit)
	...
	if targetUnit:getTraits().temporaryProtect then
		...
		self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = STRINGS.UI.FLY_TXT.PERSONAL_SHIELD, x = x1, y = y1, color = { r = 1, g = 1, b = 0, a = 1 } })
	elseif targetUnit:isDead() then
		...
		if targetUnit:getTraits().neutralize_shield then
			if targetUnit:getTraits().neutralize_shield > 0 then
				...
				self:dispatchEvent(simdefs.EV_UNIT_HIT_SHIELD,
					{ unit = targetUnit, sourceUnit = sourceUnit, shield = targetUnit:getTraits().neutralize_shield })
			end
		end

		if (targetUnit:getTraits().shields or 0) > 0 then
			self:dispatchEvent(simdefs.EV_UNIT_HIT_SHIELD,
				{ unit = targetUnit, sourceUnit = sourceUnit, shield = targetUnit:getTraits().shields })
		end

		if damage > 0 or (kodamage and kodamage > 0) then
			self:dispatchEvent(simdefs.EV_UNIT_HIT,
				{ unit = targetUnit, sourceUnit = sourceUnit, result = damage, kodamage = kodamage, fx = fx, pinned =
				simquery.isUnitPinned(self, targetUnit) })
			...
			if targetUnit:isDead() and sourceUnit and sourceUnit:countAugments("augment_sadochistic_pumps") > 0 then
				...
				self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = STRINGS.ITEMS.AUGMENTS.SADOCHISTIC_PUMPS, x = x1, y = y1, color = { r = 1, g = 1, b = 41 / 255, a = 1 } })
...
function simengine:hitUnit(sourceUnit, targetUnit, dmgt)
	...
	local anatomyAnalysisPWR = 3
	if self:isVersion("0.17.13") then -- fixes a bug where anatomy analysis needed less than 3 to trigger instead of equal or more than.
		anatomyAnalysisPWR = 2
	end
	...
	elseif sourceUnit and sourceUnit:countAugments("augment_anatomy_analysis") > 0 and sourceUnit:getPlayerOwner():getCpus() > anatomyAnalysisPWR then
	...
	self:triggerEvent(simdefs.TRG_UNIT_HIT,
		{ targetUnit = targetUnit, sourceUnit = sourceUnit, x = x1, y = y1, noTargetAlert = dmgt.noTargetAlert })
end
...
function simengine:AOEbuffFirewalls(x, y, range, buff)
	...
	self:dispatchEvent(simdefs.EV_OVERLOAD_VIZ,
		{ x = x, y = y, units = targetUnits, range = range, fx = fx, mainframe = true })

	for _, unit in ipairs(targetUnits) do
		unit:getTraits().mainframe_ice = unit:getTraits().mainframe_ice + buff
		unit:getTraits().mainframe_iceMax = unit:getTraits().mainframe_iceMax + buff
	end
end
	...
	for i, cellUnit in ipairs(cell.units) do
		if player then
			if player:isNPC() then
				if scanGrenade and self:getParams()["advanced_scanners"] then
					if cellUnit:getTraits().hologram then
						cellUnit:deactivate()
						if unit then
							self:processReactions(unit)
						end
				...
				if simquery.isEnemyAgent(player, cellUnit, ignoreDisguise) then
					if unit then
						if scanGrenade and self:getParams()["advanced_scanners"] then
							--dissable invisibility
							if cellUnit:getTraits().invisible then
								cellUnit:setInvisible(false)
							end

							--dissable holograms
							if cellUnit:getTraits().disguiseOn then
								cellUnit:setDisguise(false)
							end
						end

						if unit:getTraits().relayInterest then
							local guard = nil
							local dist = 9999

							for i, child in pairs(self:getAllUnits()) do
								if child:getTraits().isAgent and child:getPlayerOwner() and child:getPlayerOwner():isNPC() and not child:isKO() and not child:isDead() and child ~= unit then
							...
							if guard then
								guard:getBrain():getSenses():addInterest(cell.x, cell.y, simdefs.SENSE_SIGHT,
									simdefs.REASON_SCANNED, cellUnit, ignoreDisguise)
							end
							self:processReactions(unit)
						else
							unit:getBrain():getSenses():addInterest(cell.x, cell.y, simdefs.SENSE_SIGHT,
								simdefs.REASON_SCANNED, cellUnit, ignoreDisguise)
						end
					else
						self:triggerEvent(simdefs.TRG_NEW_INTEREST,
							{ x = x0, y = y0, range = simdefs.SOUND_RANGE_3, interest = { x = cell.x, y = cell.y, reason = simdefs.REASON_SCANNED } })
					end
				end
			else
				if (cellUnit:getTraits().mainframe_item or cellUnit:getTraits().mainframe_console) and not cellUnit:getTraits().scanned then
					...
					self:dispatchEvent(simdefs.EV_UNIT_REFRESH, { unit = cellUnit, reveal = true })
				end
			end
		end
	end
...
function simengine:moveDaemon(unit)
	if self._daemonQueue then
		...
		if closestItem then
			...
			if self:isVersion("0.17.9") then
				local serverdefs = include("modules/serverdefs")
				local programList = self:handleOverrideAbility(serverdefs.OMNI_PROGRAM_LIST)

				if self:getParams().difficultyOptions.daemonQuantity == "LESS" then
					programList = self:handleOverrideAbility(serverdefs.OMNI_PROGRAM_LIST_EASY)
				end

				daemon = programList[self:nextRand(1, #programList)]
			else
				...
				daemon = daemons[self:nextRand(1, #daemons)]
			end
			...
			self:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
				{ unit = closestItem, ice = closestItem:getTraits().mainframe_ice, delta = 0 })
			closestItem:getTraits().mainframe_program = daemon
			...
			self:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
				{ unit = closestItem, ice = closestItem:getTraits().mainframe_ice, delta = 0, refreshAll = true })
...
function simengine:trackerAdvance(delta, txt, scan)
	if self._trackerQueue then
			...
			for i = 1, delta do
				...
				if stage > self._trackerStage then
					...
					self:dispatchEvent(simdefs.EV_ADVANCE_TRACKER,
						{ txt = txt, delta = self._tracker - oldTracker, tracker = oldTracker })
						...
						for _, unitRaw in pairs(self:getAllUnits()) do
							if unitRaw:getTraits().safeUnit and unitRaw:getTraits().credits and unitRaw:getTraits().credits > 0 then
								...
								if player:hasSeen(unitRaw) then
									self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
										{ txt = self:getTags().protocol78, x = x0, y = y0, color = { r = 163 / 255, g = 243 / 255, b = 248 / 255, a = 1 }, skipQue = true })
								end
							end
						end
					end
					...
					for i, unit in pairs(pc:getAgents()) do
						...
						for c, child in ipairs(unit:getChildren()) do
							if child:getTraits().alarmMPmod then
								if not self:isVersion("0.17.12") or not child:getTraits().augment or child:getTraits().installed then
									self:dispatchEvent(simdefs.EV_GAIN_AP, { unit = unit })
									local x2, y2 = unit:getLocation()

									self:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
										{ txt = child:getTraits().alarmMPmodTxt, x = x2, y = y2, color = cdefs
										.AUGMENT_TXT_COLOR })
									self:dispatchEvent(simdefs.EV_UNIT_REFRESH, { unit = unit })
		...
		self._resultTable.max_alarm_level = self._resultTable.max_alarm_level and
		math.max(self._resultTable.max_alarm_level, self._tracker) or self._tracker
...
function simengine:setMissionReward(missionReward)
	if self._missionReward then
		self._missionReward = self._missionReward + missionReward
	else
...
function simengine:emitSound(sound, x0, y0, unit, altVisTiles)
	...
	if sound.range > 0 and not sound.innocuous then
		self:triggerEvent(simdefs.TRG_SOUND_EMITTED,
			{ x = x0, y = y0, range = sound.range, sourceUnit = unit, ignoreSight = sound.ignoreSight })
	end
end
