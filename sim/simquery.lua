...
function _M.canSoftPath(sim, unit, startcell, endcell)
	...
	local isDiagonal = (startcell.x + 1 == endcell.x or startcell.x - 1 == endcell.x) and
	(startcell.y + 1 == endcell.y or startcell.y - 1 == endcell.y)
...
function _M.canPathBetween(cellquery, unit, startcell, endcell)
	...
	if (startcell.x + 1 == endcell.x or startcell.x - 1 == endcell.x) and (startcell.y + 1 == endcell.y or startcell.y - 1 == endcell.y) then
		...
		local d1, d2 = _M.getDirectionFromDelta(cell3.x - startcell.x, cell3.y - startcell.y),
			_M.getDirectionFromDelta(endcell.x - cell3.x, endcell.y - cell3.y)
		local d3, d4 = _M.getDirectionFromDelta(cell4.x - startcell.x, cell4.y - startcell.y),
			_M.getDirectionFromDelta(endcell.x - cell4.x, endcell.y - cell4.y)
		...
		if unit then
			found = found and
			(not (exit.closed and exit.locked) or _M.canModifyExit(unit, simdefs.EXITOP_UNLOCK, startcell, dir))
...
function _M.getMoveCost(cell1, cell2)
	assert(math.abs(cell1.x - cell2.x) <= 1 and math.abs(cell1.y - cell2.y) <= 1 or
	error(string.format("<%d, %d> -> <%d, %d>", cell1.x, cell1.y, cell2.x, cell2.y)))
...
function _M.calculateDamageAndArmorForMelee(sim, sourceUnit, targetUnit)
	...
	if _M.isAgent(unitOwner) then
		...
		-- KINETIC CAPACITOR
		if unitOwner:getTraits().kinetic_capacitor_bonus and unitOwner:getTraits().kinetic_capacitor_bonus_charged then
	...
	if targetUnit then
		armor = targetUnit:getArmor()
	end

	return meleeDamage, armorPiercing, armor
end
...
function _M.calculateMeleeDamage(sim, sourceUnit, targetUnit)
	...
	local meleeDamage, armorPiercing, armor = _M.calculateDamageAndArmorForMelee(sim, sourceUnit, targetUnit)
