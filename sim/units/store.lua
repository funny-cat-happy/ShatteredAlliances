...
local function createStoreItems( store, storeUnit, sim )
	...
	--Take out items that should be overwriten by mods
	if sim:isVersion("0.17.18") then
		for i=#itemsList,1,-1 do
			if itemsList[i].markedForDeletion then
				table.remove(itemsList,i)
			end
		end
		for i=#weaponsList,1,-1 do
			if weaponsList[i].markedForDeletion then
				table.remove(weaponsList,i)
			end
		end
		for i=#augmentsList,1,-1 do
			if augmentsList[i].markedForDeletion then
	...
	print("^^^###^^^")
	for i, item in ipairs(weaponsList)do
...
return
{
	...
	createProgramUnitData = createProgramUnitData,
}
