	...
	if self:getTraits().range then
		...
		for i, cell in ipairs(cells) do
			for i, cellUnit in ipairs( cell.units ) do
				...
				local localdmgt = dmgt

 				-- this loop was using the same dmgt and reducing the damage with each guards resistance getting less and less by the end.				
 				-- this makes a new damage table for each target in the loop
				if sim:isVersion("0.17.12") then
					localdmgt = util.tcopy(dmgt)
				end
				...
				if simquery.isEnemyAgent(self:getPlayerOwner(), cellUnit) and not cellUnit:isKO() then
					if cellUnit:getTraits().canKO then
				    	if sim:isVersion("0.17.5") then
							localdmgt.damage = unit:processKOresist( localdmgt.damage )
				    	end						
						sim:hitUnit( self, cellUnit, localdmgt )
		...
		-- user is handed by the radius is it not?
		-- don't need this for the radius effect.
		if not sim:isVersion("0.17.12") then 
	        if unit then	        	
