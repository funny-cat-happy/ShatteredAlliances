...
local CEO_ESCAPED = 
{
	trigger = "vip_escaped",
    fn = function( sim, evData )
    	if evData.unit:getTraits().cfo then
    		return true
    	end
    end,
}
...
local PC_KNOCKOUT_CEO =
{
    ...
    fn = function( sim, triggerData )
        if triggerData and (triggerData.ticks or 0) > 0 then
        	if (not sim:isVersion("0.17.12") and triggerData.unit:getTraits().ko_trigger == "intimidate_guard") or triggerData.unit:getTraits().cfo  then 
...
local function ceoalerted(script, sim, mission)
	...
	if not ceo:isKO() and not sim:hasTag( "cardEjected") then
		...
		script:addHook( script.hookFn, nil, mission)
	end 
end
