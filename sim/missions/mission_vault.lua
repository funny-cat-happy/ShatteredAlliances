...
local function hackvault(script, sim, mission)
    ...
    script:queue({
        script = SCRIPTS.INGAME.AFTERMATH.VAULT[sim:nextRand(1, #SCRIPTS.INGAME.AFTERMATH.VAULT)],
        type =
        "newOperatorMessage"
    })
end
...
function mission:init(scriptMgr, sim)
    ...
    scriptMgr:addHook("SEENANO",
        mission_util.DoReportObject(mission_util.PC_SAW_UNIT_WITH_TRAIT("open_secure_boxes"),
            SCRIPTS.INGAME.SEEOBJECTIVE.VAULT))
