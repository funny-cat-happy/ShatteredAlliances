...
local mission_util =
{
    ...
    EV_TALKING_HEAD_CLOSED =
    {
	    uiEvent = level.EV_TALKING_HEAD_CLOSED,
    },
	...
	PC_TOOK_UNIT_WITH_TAG = function( tag )
		return
		{
			...
			fn = function( sim, unitID, itemID )
				...
				print(item:getName(),tag,item:hasTag(tag))
	...
	ALARM_STATE_CHANGE = function( level )
		return
		{
			trigger = simdefs.TRG_ALARM_STATE_CHANGE,
			fn = function( sim, evData )
				if not level or level == evData then
					return true
				end
			end,
		}
	...
	TRG_UNIT_DEACTIVATED = function()
		return
		{
			trigger = simdefs.TRG_UNIT_DISABLED,
			fn = function( sim, evData )
				return evData
			end,
		}
    ...
    PC_SAW_CELL_WITH_TAG = function(script, tag )
        return
        {
...
mission_util.checkGameOver = function( script, sim )
	...
	print("FAIL MESSAGE",sim:getTags().failMessage)
	if sim:hasTag("failMessage") then
		script:queue( { script=sim:getTags().failMessage, type="modalConversation" } )  
	else
...
local function scannerDetected( script, sim )
    ...
    for i,unit in pairs(sim:getAllUnits())do
        if unit:getTraits().scanner and unit:getPlayerOwner() ~= sim:getPC() and unit:getTraits().mainframe_status == "active" then
            scanners = scanners + 1
        end
    end
    ...
    if scannerUnit then
    	...
    	if scanners == 1 then
    		...
    		script:queue( { script=SCRIPTS.INGAME.CENTRAL_SCANNERS_DETECTED, type="newOperatorMessage" } )		
    	end
        ...
        if scanners == 1 then
           	sim:addObjective( STRINGS.MISSIONS.ESCAPE.OBJ_SCANNER, "scanner", num )
        else
   			sim:addObjective( STRINGS.MISSIONS.ESCAPE.OBJ_SCANNERS, "scanner", num )
   		end        	
 		...
 		if scanners > 1 then
            while scanners > 0 do
                ...
                for i,unit in pairs(sim:getAllUnits())do
                    if unit:getTraits().scanner and unit:getPlayerOwner() ~= sim:getPC() and unit:getTraits().mainframe_status == "active" then                                
                        newScanners = newScanners + 1
                    end                   
                end    
  
                if newScanners < scanners then        
    	            sim:incrementTimedObjective( "scanner" )
	                scanners = scanners -1
            	end
            end
	    ...
	    script:queue( 2*cdefs.SECONDS )
 		...
 		if scanners == 1 then
   			...
   			script:queue( { script=SCRIPTS.INGAME.CENTRAL_SCANNERS_DETECTED_2, type="newOperatorMessage" } )	
   		end
    end
end
