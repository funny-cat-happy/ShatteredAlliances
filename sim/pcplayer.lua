...
function pcplayer:rescueHostage( sim, unit, cell, facing, userUnit )
	...
	if unit:getTraits().tinker_anim then
		userUnit:setFacing(tempfacing)	
		sim:dispatchEvent( simdefs.EV_UNIT_USECOMP, { unitID = userUnit:getID(), useTinker=true, facing = tempfacing, sound = "SpySociety/Actions/hostage/free_hostage" , soundFrame = 16 } )		
	end	
