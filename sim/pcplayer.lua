	...
	sim:forEachCell(
		function(c)
			if c.sightRadius or c.deployID == simdefs.DEFAULT_EXITID then
				local cells = simquery.floodFill(sim, nil, c, math.max(c.sightRadius or 0, 6),
...
function pcplayer:rescueHostage(sim, unit, cell, facing, userUnit)
	...
	if unit:getTraits().untie_anim then
		...
		sim:dispatchEvent(simdefs.EV_UNIT_USECOMP,
			{ unitID = userUnit:getID(), facing = tempfacing, sound = "SpySociety/Actions/hostage/free_hostage", soundFrame = 16 })
		sim:dispatchEvent(simdefs.EV_UNIT_UNTIE, { unit = unit })
	end

	if unit:getTraits().tinker_anim then
		userUnit:setFacing(tempfacing)
		sim:dispatchEvent(simdefs.EV_UNIT_USECOMP,
			{
				unitID = userUnit:getID(),
				useTinker = true,
				facing = tempfacing,
				sound =
				"SpySociety/Actions/hostage/free_hostage",
				soundFrame = 16
			})
	end
	...
	newUnit = simfactory.createUnit(unitdefs.agent_templates[template], sim)
