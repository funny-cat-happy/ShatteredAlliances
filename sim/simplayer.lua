...
function findMainframeAbility(abilityID)
...
function simplayer:hasMainframeAbility(abilityID)
	if abilityID then
		local test = true
		while test do
			local testAbility = findMainframeAbility(abilityID)
			if testAbility.abilityOverride then --sim:isVersion(testAbility.abilityOverride[2])
				abilityID = testAbility.abilityOverride[1]
			end
			testAbility = findMainframeAbility(abilityID)
			if not testAbility.abilityOverride then
				test = false
			end
		end
	end
	...
	if self._mainframeAbilities then
		for i, ability in ipairs(self._mainframeAbilities) do
			local ownerAbilityID = ability:getID()
			local test = true
			while test do
				local testAbility = findMainframeAbility(ownerAbilityID)
				if testAbility.abilityOverride then --sim:isVersion(testAbility.abilityOverride[2])
					ownerAbilityID = testAbility.abilityOverride[1]
				end
				testAbility = findMainframeAbility(ownerAbilityID)
				if not testAbility.abilityOverride then
					test = false
				end
			end

			if ownerAbilityID == abilityID then
				return ability
			end
		end
	end
...
function simplayer:getEscapedWithObjective()
	if self._escapedWithObjective then
		return true
	else
		return false
	end
...
function simplayer:setEscapedWithObjective(bool)
	self._escapedWithObjective = bool
end

function simplayer:addCredits(credits, sim, x, y)
	self._credits = math.floor(self._credits + credits)

	local sound = "SpySociety/HUD/gameplay/gain_money"
	...
	if self._sim and sound then
		self._sim:dispatchEvent(simdefs.EV_PLAY_SOUND, sound)
	end

	if x and y then
		self._sim:dispatchEvent(simdefs.EV_UNIT_FLY_TXT,
			{ txt = util.sformat(STRINGS.FORMATS.CREDITS, credits), x = x, y = y, color = { r = 1, g = 1, b = 1, a = 1 }, target =
			"credits" })
	end
end
...
function simplayer:addCPUs(delta, sim, x, y, delay)
	...
	if actual_delta > 0 then
		self._sim._resultTable.pwr_gained = self._sim._resultTable.pwr_gained and
		self._sim._resultTable.pwr_gained + delta or delta
	elseif actual_delta < 0 then
		self._sim._resultTable.pwr_used = self._sim._resultTable.pwr_used and self._sim._resultTable.pwr_used - delta or
		-delta
	end
	...
	if x and y then
		...
		sim:dispatchEvent(simdefs.EV_UNIT_FLY_TXT,
			{ txt = floatTxt, x = x, y = y, color = { r = 1, g = 1, b = 1, a = 1 }, sound = sound, soundDelay = 0.1, delay =
			delay })                                                                                                                           --1.5
	else
	...
	for i, unit in ipairs(units) do
		...
		for j, mainframeUnit in ipairs(mainframeItems) do
			...
			if distance <= unit:getTraits().firewallSupportRange then
				...
				mainframeUnit:getTraits().mainframe_ice = math.min(mainframeUnit:getTraits().mainframe_ice + 2,
					mainframeUnit:getTraits().mainframe_iceMax + 2)
				sim:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
					{ unit = mainframeUnit, ice = mainframeUnit:getTraits().mainframe_ice, delta = 2 })
			end
		end
	end
end
...
function simplayer:deployUnits(sim, agentDefs)
	...
	for _, agentDef in ipairs(agentDefs) do
		local i
		deploy_cell, i = array.findIf(deploycells,
			function(c) return c.deployID == (agentDef.deployID or simdefs.DEFAULT_EXITID) end)
		if deploy_cell then
