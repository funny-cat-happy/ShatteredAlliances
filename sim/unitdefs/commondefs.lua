...
local function onFooterTooltip( tooltip, unit, userUnit )
...
local function onAgentTooltip( tooltip, unit )
	...
	if unit:isPC() then
		...
		for i,tooltipFunction in ipairs(mod_manager:getTooltipDefs().onAgentTooltips)do
...
local item_template = 
{
    ...
    onTooltip = function( tooltip, unit, userUnit )
    	...
    	onFooterTooltip( tooltip, unit, userUnit )
    end,
}
...
local function onGuardTooltip( tooltip, unit )
    ...
    if (traits.LOSperipheralArc or 0) == (2 * math.pi) then    	
        tooltip:addAbility( STRINGS.UI.TOOLTIPS.ADVANCED_PERIPHERAL_BOOST, STRINGS.UI.TOOLTIPS.ADVANCED_PERIPHERAL_BOOST_DESC,  "gui/icons/arrow_small.png"  )
    else
...
--operatable_device
    ...
    for i,tooltipFunction in ipairs(mod_manager:getTooltipDefs().onMainframeTooltips)do
		tooltipFunction(tooltip, unit )
	end       
end
