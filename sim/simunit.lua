...
function simunit:addChild( childUnit )
	...
	if childUnit:getTraits().mp_penalty and self:getSim() then
		...
		self:getSim():dispatchEvent( simdefs.EV_UNIT_FLOAT_TXT, { txt=util.sformat(STRINGS.UI.FLY_TXT.HEAVY,childUnit:getTraits().mp_penalty), x=x1,y=y1, color={r=163/255,g=0,b=0,a=1}, skipQue=false } ) 
	end	
...
function simunit:onStartTurn( sim )
    ...
    if self:getPlayerOwner() ~= nil then
	    ...
	    if power > 0 then
	    	sim:dispatchEvent( simdefs.EV_UNIT_FLOAT_TXT, {txt = util.sformat( STRINGS.UI.FLY_TXT.PLUS_PWR, power), unit = self, color={r=255/255,g=255/255,b=51/255,a=1}} )
...
function simunit:onEndTurn( sim )
    ...
    if sim:getCurrentPlayer() == self:getPlayerOwner() then
	    if traits.mpMax then
			...
			local x1,y1 = self:getLocation()
				self:getSim():dispatchEvent( simdefs.EV_UNIT_FLOAT_TXT, { txt=util.sformat(STRINGS.UI.FLY_TXT.HEAVY, totalPenalty ), x=x1,y=y1, color={r=163/255,g=0,b=0,a=1}, skipQue=false } ) 
			end
