...
function simunit:getAugmentCount()
	...
	for _, childUnit in ipairs(self:getChildren()) do
		if childUnit:hasAbility("carryable") and childUnit:getTraits().augment and childUnit:getTraits().installed then
			if self:getSim():isVersion("0.17.14") or not childUnit:getTraits().ingnoreAugmentCount17_13 then
				count = count + 1
			end
		end
	end
...
function simunit:countArmorPiercingUpgrades(traitName)
	...
	if self._children then
		for i, child in ipairs(self._children) do
			-- fix for augments in inventory giving pirce boost		
			if self._sim:isVersion("0.17.16") then
				if child:getTraits()[traitName] and (not child:getTraits().augment or child:getTraits().installed) then
...
function simunit:addChild(childUnit)
	...
	if childUnit:getTraits().mp_penalty and self:getSim() then
		self:useMP(childUnit:getTraits().mp_penalty, self:getSim())
		local x1, y1 = self:getLocation()
		self:getSim():dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = util.sformat(STRINGS.UI.FLY_TXT.HEAVY, childUnit:getTraits().mp_penalty), x = x1, y = y1, color = { r = 163 / 255, g = 0, b = 0, a = 1 }, skipQue = false })
	end
		...
		if facing ~= self:getFacing() then
			...
			if floatTxt then
				sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = floatTxt, x = x1, y = y1, color = { r = 1, g = 1, b = 41 / 255, a = 1 }, skipQue = true })
			end
		end
	end
end
...
function simunit:processKOresist(damage)
	...
	if self:getPlayerOwner() and self:getPlayerOwner():getTraits().boostKOresist and not self:getTraits().isDrone then
...
function simunit:stopHacking(sim)
	if self:getTraits().data_hacking then
		local target = sim:getUnit(self:getTraits().data_hacking)
		if target then
...
function simunit:useAP(sim)
	...
	if self:getTraits().actionAP then
		...
		if self:getPlayerOwner() ~= sim:getCurrentPlayer() then
			...
			table.insert(self:getTraits().floatTxtQue,
				{ txt = util.sformat(STRINGS.UI.FLY_TXT.ADRENAL_REGULATOR, ADRENAL_BONUS), color = { r = 255 / 255, g = 178 / 255, b = 102 / 255, a = 1 } })
		else
			local x1, y1 = self:getLocation()
			sim:dispatchEvent(simdefs.EV_GAIN_AP, { unit = self })
			sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ unit = self, txt = util.sformat(STRINGS.UI.FLY_TXT.ADRENAL_REGULATOR, ADRENAL_BONUS), x = x1, y = y1, color = { r = 255 / 255, g = 178 / 255, b = 102 / 255, a = 1 } })      --
		end
...
function simunit:checkOverload(sim)
	if self:getTraits().inventoryMaxSize then
		...
		if overload > (self:getTraits().overloadCount or 0) then
			...
			local x1, y1 = self:getLocation()
			sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = util.sformat(STRINGS.UI.FLY_TXT.ENCUMBERED, overload), x = x1, y = y1, color = { r = 163 / 255, g = 0, b = 0, a = 1 } })
...
function simunit:processEMP(bootTime, noEmpFX)
	...
	elseif self:getTraits().mainframe_status ~= nil and self:getTraits().mainframe_status ~= "off" then
		...
		if self:getTraits().magnetic_reinforcement and self:getTraits().mainframe_ice > 2 then
			...
			self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = STRINGS.UI.TOOLTIPS.MAGNETIC_REINFOREMENTS, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 255 / 255, a = 1 } })
		else
	...
	if doEMPeffect then
		if not noEmpFX then
			...
			self._sim:dispatchEvent(simdefs.EV_PLAY_SOUND,
				{ sound = "SpySociety/HitResponse/hitby_distrupter_flesh", x = x0, y = y0 })
	...
	if not empResisted and self:getTraits().heartMonitor == "enabled" then
		...
		if self:getTraits().improved_heart_monitor then
			self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = STRINGS.UI.FLY_TXT.IMPROVED_HEART_MONITOR, x = x1, y = y1, color = { r = 1, g = 1, b = 41 / 255, a = 1 } })
		else
			self:getTraits().heartMonitor = "disabled"
			self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = STRINGS.UI.FLY_TXT.MONITOR_DISABLED, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 255 / 255, a = 1 } })
		end
	end
...
function simunit:setTagged(dissable)
	if dissable then
		...
		local x1, y1 = self:getLocation()
		self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = STRINGS.UI.FLY_TXT.TAG_REMOVED, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 255 / 255, a = 1 } })
	else
		...
		local x1, y1 = self:getLocation()
		self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = STRINGS.UI.FLY_TXT.TAGGED, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 255 / 255, a = 1 } })
...
function simunit:setDisguise(state, kanim, noReaction)
	local sim = self._sim
	...
	if (self:getTraits().disguiseOn == true) ~= (state == true) then
		...
		if kanim then
			...
			self._sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = STRINGS.UI.FLY_TXT.DISGUISED, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 255 / 255, a = 1 } })
			sim:dispatchEvent(simdefs.EV_UNIT_ADD_FX,
				{ unit = self, kanim = "fx/agent_cloak_fx", symbol = "effect", anim = "in", above = true, params = params })
			...
			sim:dispatchEvent(simdefs.EV_UNIT_ADD_FX,
				{ unit = self, kanim = "fx/agent_cloak_fx", symbol = "effect", anim = "out", above = true, params =
				params })
			...
			if not noReaction or not sim:isVersion("0.17.9") then
				sim:processReactions(self)
			end
		end
	end
...
function simunit:setInvisible(state, duration)
	...
	if (self:getTraits().invisible == true) ~= (state == true) then
		...
		if self:getTraits().invisible then
			...
			sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = STRINGS.ABILITIES.CLOAKED, x = x2, y = y2, color = { r = 255 / 255, g = 255 / 255, b = 51 / 255, a = 1 } })

			if self:countAugments("augment_chameleon_movement") > 0 then
				sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = STRINGS.ITEMS.AUGMENTS.CHAMELEON_MOVEMENT, x = x2, y = y2, color = { r = 255 / 255, g = 255 / 255, b = 51 / 255, a = 1 } })
...
function simunit:buffArmor(sim, armorBuff)
	local x1, y1 = self:getLocation()
	sim:dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = self:getTraits().buffArmorOnKO_sfx, x = x1, y = y1 })
	...
	sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
		{ txt = util.sformat(STRINGS.UI.FLY_TXT.ARMOR_UP, armorBuff), x = x1, y = y1, color = { r = 1, g = 1, b = 41 / 255, a = 1 } })
	local params = { color = { { symbol = "wall", r = 1, g = 0, b = 0, a = 1 } } }
	sim:dispatchEvent(simdefs.EV_UNIT_ADD_FX,
		{ unit = self, kanim = "fx/firewall_buff_fx_2", symbol = "character", anim = "in", above = true, params = params })
end

function simunit:AOEFirewallsBuff(sim)
	local x1, y1 = self:getLocation()
	local range = self:getTraits().AOEFirewallsBuffRange
	local buff = self:getTraits().AOEFirewallsBuff
	if self:getTraits().AOEFirewallsBuff_sfx then
		sim:dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = self:getTraits().AOEFirewallsBuff_sfx, x = x1, y = y1 })
	end
	...
	elseif self:getTraits().koTimer ~= ticks then
		...
		if ticks then
			...
			if self:getTraits().consciousness_monitor then
				...
				local x0, y0 = self:getLocation()
				sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = STRINGS.UI.FLY_TXT.CONSCIOUSNESS_MONITOR, x = x0, y = y0, color = { r = 1, g = 1, b = 41 / 255, a = 1 } })
			...
			if self:getTraits().shieldArmor then
				...
				local x0, y0 = self:getLocation()
				sim:dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = "SpySociety/Actions/mainframe_object_on", x = x0, y =
				y0 })
			end
		end
			...
			if self:getPlayerOwner() ~= sim:getPC() then
				...
				for i, unit in pairs(pc:getAgents()) do
					...
					if count > 0 then
						...
						sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
							{ txt = augments[1]:getTraits().boost_AP_on_wakeup_txt, x = x2, y = y2, color =
							AUGMENT_TXT_COLOR })
...
function simunit:killUnit(sim)
	...
	if self:getTraits().heartMonitor then
		...
		if alarmInc > 0 then
			sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = util.sformat(STRINGS.UI.ALARM_ADD, alarmInc), x = x0, y = y0, color = { r = 255 / 255, g = 10 / 255, b = 10 / 255, a = 1 } })
			sim:trackerAdvance(alarmInc, text)
		end
	end
	...
	if self:getTraits().mainframe_device then
		...
		sim:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
			{ unit = mainFrameDevice, ice = mainFrameDevice:getTraits().mainframe_ice, delta = 0 })
		...
		sim:dispatchEvent(simdefs.EV_UNIT_ADD_FX,
			{ unit = mainFrameDevice, kanim = "fx/deamon_ko", symbol = "effect", anim = "break", above = true, params =
			params })
...
function simunit:increaseIce(sim, iceInc)
	if self:getTraits().mainframe_iceMax then
		sim:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
			{ unit = self, ice = self:getTraits().mainframe_ice, delta = iceInc })
...
function simunit:scanCell(sim, cell)
	...
	for i, cellUnit in ipairs(cell.units) do
		if player:isNPC() then
			if simquery.isEnemyAgent(player, cellUnit, true) then
				local guard = nil
				local dist = 9999
				...
				if guard then
					guard:getBrain():getSenses():addInterest(cell.x, cell.y, simdefs.SENSE_SIGHT, simdefs.REASON_SCANNED,
						cellUnit)
				end
			end
		else
...
function simunit:onStartTurn(sim)
	...
	if self:getPlayerOwner() ~= nil then
		...
		if power > 0 then
			sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
				{ txt = util.sformat(STRINGS.UI.FLY_TXT.PLUS_PWR, power), unit = self, color = { r = 255 / 255, g = 255 / 255, b = 51 / 255, a = 1 } })
...
function simunit:resetSwitch(sim)
	local traits = self:getTraits()
	traits.switched = nil
	if traits.mainframe_status ~= "off" then
		traits.mainframe_status = "active"
		local x0, y0 = self:getLocation()
		sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
			{ txt = traits.switch_reset_text, x = x0, y = y0, color = { r = 1, g = 1, b = 41 / 255, a = 1 }, skipQue = true })
		if self:getSounds().switch_reset then
			sim:dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = self:getSounds().switch_reset, x = x0, y = y0 })
		end
		if self:getSounds().computer_reset then
			sim:dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = self:getSounds().computer_reset, x = x0, y = y0 })
		end
...
function simunit:returnItemsToStash(sim)
	local originalUnitData = util.tcopy(self:getUnitData())

	for i, item in ipairs(self:getChildren()) do
		--put item in inventory, minus starting equipment/augments installed.
		if not sim._params.agency.upgrades then
			sim._params.agency.upgrades = {}
		end
		local original = false
		for i, child in ipairs(originalUnitData.children) do
			print("Testomg", child.id, item:getUnitData().id)
			if child.id == item:getUnitData().id then
				print("THIS ITEM IS ORIGINAL")
				original = i
			end
		end

		if original then
			table.remove(originalUnitData.children, original)
		end

		if not item:getTraits().installed and not original then
			...
			if upgradeName then
				...
				table.insert(sim._params.agency.upgrades, { upgradeName = upgradeName, upgradeParams = upgradeParams })
			end
		end
	end
end
...
function simunit:onEndTurn(sim)
		...
		if traits.mpMax then
			...
			for i, unit in ipairs(self:getChildren()) do
				if unit:getTraits().mp_penalty then
					traits.mp = traits.mp - unit:getTraits().mp_penalty
					totalPenalty = totalPenalty + unit:getTraits().mp_penalty
				end
			end
			if totalPenalty > 0 then
				local x1, y1 = self:getLocation()
				self:getSim():dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
					{ txt = util.sformat(STRINGS.UI.FLY_TXT.HEAVY, totalPenalty), x = x1, y = y1, color = { r = 163 / 255, g = 0, b = 0, a = 1 }, skipQue = false })
			end
	...
	if traits.switched and not traits.unlocked then
		self:resetSwitch(sim)
		if traits.mainframe_status ~= "off" then
			sim:triggerEvent(simdefs.TRG_MAP_EVENT, { event = simdefs.MAP_EVENTS.RESET_SWITCH })
			for i, childunit in pairs(sim:getAllUnits()) do
				if childunit.switched then
					childunit:resetSwitch(sim)
				end
			end
		end
	end
...
function simunit:progressHack()
	local sim = self:getSim()
	self:getTraits().progress = self:getTraits().progress + 1

	if self:getSounds().stageAdvance then
		local x1, y1 = self:getLocation()
		self:getSim():dispatchEvent(simdefs.EV_PLAY_SOUND, { sound = self:getSounds().stageAdvance, x = x1, y = y1 })
	end
	self:getSim():dispatchEvent(simdefs.EV_UNIT_SWTICH_FX, { unit = self, transition = true })


	sim:triggerEvent("databank_hack", { unit = self })

	if self:getTraits().progress == self:getTraits().progressMax then
		local hacker = sim:getUnit(self:getTraits().hacker)
		sim:triggerEvent("databank_hack_end", { unit = self, hacker = hacker })
		hacker:getTraits().data_hacking = nil
		hacker:getSounds().spot = nil
		sim:dispatchEvent(simdefs.EV_UNIT_TINKER_END, { unit = hacker })
		sim:dispatchEvent(simdefs.EV_UNIT_REFRESH, { unit = hacker })
		self:getTraits().progress = nil
		self:getTraits().mainframe_status = "off"
		sim:dispatchEvent(simdefs.EV_UNIT_REFRESH, { unit = self })
	end
end
			...
			if checkUnit:getTraits().hologram then
				...
				if x0 and y0 and x1 and y1 then
					...
					if distance < checkDist then
						...
						if brain then
							brain:getSenses():addInterest(x1, y1, simdefs.SENSE_HEARING, simdefs.REASON_FOUNDOBJECT,
								checkUnit)
						end
					end
				end
...
function simunit:removeSeenUnit(unit, oldcell, newcell)
	...
	if unit:getTraits().sightable then
		self._sim:triggerEvent(simdefs.TRG_UNIT_DISAPPEARED,
			{ seerID = self:getID(), unit = unit, from_cell = oldcell, to_cell = newcell })
	end
end
...
function simunit:canBeInterrupted()
	...
	return self:getTraits().movePath or self:getTraits().modifyingExit or self:getTraits().lookingAround or
	self:getTraits().throwing
end
...
function simunit.createUnit(unitData, sim, ...)
	if sim and unitData.upgradeOverride and sim:isVersion(unitData.upgradeOverride[1]) then
		unitData = unitdefs.lookupTemplate(unitData.upgradeOverride[2])
	end
	...
	if unitData then
		...
		if unitData.traits.tier2safe then
			-- added map intel data to larege safes for the Subdermal PDA		
			if sim:isVersion("0.17.5") then
		...
		if unitData.traits.generateAugment then
			-- changes augment machines to generate the augment at the start of the level
			if sim:isVersion("0.17.7") then
				local simstore = include("sim/units/store")
				unitData.traits.generateAugment = nil
				local augmentList = util.weighted_list()
				for i, augment in ipairs(simstore.STORE_ITEM.augmentList) do
					augmentList:addChoice(augment.id, augment.traits.grafterWeight or 10)
				end
				t._traits.generateAugment = augmentList:getChoice(sim:nextRand(1, augmentList:getTotalWeight()))
			end
		end
