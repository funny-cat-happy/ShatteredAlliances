...
local sprint = 
	{
		...
		executeAbility = function( self, sim, unit )
			...
			if unit:getTraits().sneaking then
				...
				if unit:hasTrait("kinetic_capacitor_bonus") then 
					unit:getTraits().kinetic_capacitor_bonus_charged = true
				end
				...
				if unit:getTraits().kinetic_capacitor_bonus_charged then
