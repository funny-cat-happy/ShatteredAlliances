...
local default_npc_abilities =
{
		...
		onSpawnAbility = function( self, sim, player )
			...
			for _, unit in ipairs(npcPlayer:getUnits() ) do
				if unit:getBrain() and not unit:getTraits().enforcer then
					if not sim:isVersion("0.17.9") or not unit:getTraits().pacifist then
	...
	creditTaker = util.extend( createDaemon( STRINGS.DAEMONS.FELIX ) )
	{
		...
		overrideAbility = {version="0.17.5", override="creditTaker_new" },
	},	
			...
			for i, unit in pairs(sim:getAllUnits()) do
				if unit:getTraits().safeUnit and unit:getPlayerOwner() ~= sim:getPC() and not unit:getTraits().open and unit:getTraits().mainframe_ice then
					...
					if sim:isVersion("0.17.7") then
						unit:getTraits().mainframe_iceMax = unit:getTraits().mainframe_iceMax * self.mult
					end
		...
		onDespawnAbility = function( self, sim )		
			...
			for i, unit in pairs(sim:getAllUnits()) do
				if unit:getTraits().safeUnit and unit:getPlayerOwner() ~= sim:getPC() and not unit:getTraits().open and unit:getTraits().mainframe_ice then
					...
					if sim:isVersion("0.17.7") then
						unit:getTraits().mainframe_ice = math.floor(unit:getTraits().mainframe_ice / self.mult)
						if unit:getTraits().mainframe_ice < 1 then
							unit:getTraits().mainframe_ice = 1
						end
						unit:getTraits().mainframe_iceMax = math.floor(unit:getTraits().mainframe_iceMax / self.mult)
						if unit:getTraits().mainframe_iceMax < 1 then
							unit:getTraits().mainframe_iceMax = 1
						end
					else
...
for k, v in pairs(default_npc_abilities) do
    assert( v.name ) -- DO NOT ADD ANYTHING TO THE TABLE THAT IS NOT AN ABILITY.
end

function ResetDaemonDefs()
	log:write("ResetDaemonDefs()")	
	util.tclear(npc_abilities)
	util.tmerge(npc_abilities, default_npc_abilities)
end
