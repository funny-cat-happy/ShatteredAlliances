...
local default_mainframe_abilities =
{
			...
			executeAbility = function(self, sim)
				...
				if sim:getParams().missionCount == 0 and sim:getTurnCount() == 2 and not sim:getTags().isTutorial then
					...
					sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
						{
							txt = STRINGS.PROGRAMS.POWER_DRIP.WARNING,
							color = cdefs.COLOR_PLAYER_WARNING,
							sound =
							"SpySociety/Actions/mainframe_gainCPU",
							icon = self.icon
						})
					player:addCPUs(1)
				end
	...
	emergency_drip = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim)
				...
				if not player:isNPC() and player:getCpus() < self.MIN_CPU and self.cooldown <= 0 then
					sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
						{
							txt = util.sformat(STRINGS.PROGRAMS.EMERGENCY_DRIP.WARNING, 5),
							color = cdefs
								.COLOR_PLAYER_WARNING,
	...
	mainframePing = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim, unit, userUnit, targetCell)
				...
				sim:dispatchEvent(simdefs.EV_SCANRING_VIS,
					{ x = cellx, y = celly, range = simdefs.SOUND_MAINFRAME_PING.range })
	...
	fusion = util.extend(DEFAULT_ABILITY)
		{
			...
			abilityOverride = { "fusion_17_10", "0.17.10" },
			...
			onTrigger = function(self, sim, evType, evData)
				...
				if evType == simdefs.TRG_START_TURN and evData:isPC() then
					if self.cooldown > 0 then
						...
						sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
							{
								txt = STRINGS.PROGRAMS.FUSION.WARNING,
								color = cdefs.COLOR_PLAYER_WARNING,
								sound =
								"SpySociety/Actions/mainframe_gainCPU",
								icon = self.icon
	...
	fusion_17_10 = util.extend(DEFAULT_ABILITY)
		{
	...
	seed = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim)
				...
				sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
					{
						txt = STRINGS.PROGRAMS.SEED.WARNING,
						color = cdefs.COLOR_PLAYER_WARNING,
						sound =
						"SpySociety/Actions/mainframe_gainCPU",
						icon = self.icon
	...
	faust = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim)
				...
				if not player:isNPC() then
					...
					sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
						{
							txt = STRINGS.PROGRAMS.FAUST.WARNING,
							color = cdefs.COLOR_PLAYER_WARNING,
							sound =
							"SpySociety/Actions/mainframe_gainCPU",
							icon = self.icon
	...
	wildfire = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim, unit, userUnit, targetCell)
				...
				sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
					{
						txt = STRINGS.PROGRAMS.WILDFIRE.WARNING,
						color = cdefs.COLOR_PLAYER_WARNING,
						sound =
						"SpySociety/Actions/mainframe_gainCPU",
						icon = self.icon
				...
				for _, unit in pairs(sim:getAllUnits()) do
					if unit:getPlayerOwner() == player and simquery.isAgent(unit) then
						...
						sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
							{ txt = util.sformat(STRINGS.FORMATS.PLUS_AP, 2), x = x0, y = y0, color = { r = 1, g = 1, b = 1, a = 1 } })
						...
						sim:dispatchEvent(simdefs.EV_UNIT_FLOAT_TXT,
							{ txt = STRINGS.PROGRAMS.WINGS.NAME, x = x1, y = y1, color = { r = 255 / 255, g = 255 / 255, b = 51 / 255, a = 1 } })
					end
				end
			end,
		},
	...
	dataBlast = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim, unit, userUnit, targetCell)
				...
				sim:dispatchEvent(simdefs.EV_OVERLOAD_VIZ,
					{ x = cellx, y = celly, units = targetUnits, range = self.range })
	...
	taurus = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim, targetUnit)
				...
				if targetUnit:getTraits().daemonHost then
					...
					local possibleUnits = {}
					for _, unit in pairs(sim:getAllUnits()) do
						if unit:getTraits().mainframe_item and unit:getTraits().mainframe_ice and unit:getTraits().mainframe_ice > 0 and unit:getPlayerOwner() ~= sim:getPC() and not unit:getTraits().mainframe_program then
							table.insert(possibleUnits, unit)
						end
					end
					...
					for k = 1, 1, 1 do
						if #possibleUnits > 0 then
							...
							sim:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
								{ unit = unit, ice = unit:getTraits().mainframe_ice, delta = 0, refreshAll = true })
							...
							sim:dispatchEvent(simdefs.EV_UNIT_UPDATE_ICE,
								{ unit = unit, ice = unit:getTraits().mainframe_ice, delta = 0 })
						end
					end
				end
				if sim:isVersion("0.17.6") then
			...
			onTrigger = function(self, sim, evType, evData)
				if evType == simdefs.TRG_START_TURN and evData:isPC() then
					if self.cooldown > 0 then
						...
						sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
							{
								txt = STRINGS.PROGRAMS.ROOT.WARNING,
								color = cdefs.COLOR_PLAYER_WARNING,
								sound =
								"SpySociety/Actions/mainframe_PWRreverse_off",
								icon = self.icon
							})
					end
				end
	...
	rogue = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim, unit, userUnit, targetCell)
				...
				if #availableSafes > 0 then
					...
					sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
						{
							txt = STRINGS.PROGRAMS.ROGUE.WARNING,
							color = cdefs.COLOR_PLAYER_WARNING,
							sound =
							"SpySociety/Actions/mainframe_wisp_activate",
							icon = self.icon
						})
	...
	lightning = util.extend(DEFAULT_ABILITY)
		{
			...
			canUseAbility = function(self, sim, abilityOwner, targetUnit)
				if targetUnit then
					...
					if not sim:isVersion("0.17.9") and targetUnit:getTraits().mainframe_ice and targetUnit:getTraits().mainframe_ice > 0 then
						return false
					end
				end
	...
	dynamo = util.extend(DEFAULT_ABILITY)
		{
			...
			executeAbility = function(self, sim)
				...
				if not player:isNPC() and sim._trackerStage > 1 then
					...
					sim:dispatchEvent(simdefs.EV_SHOW_WARNING,
						{
							txt = STRINGS.PROGRAMS.DYNAMO.WARNING,
							color = cdefs.COLOR_PLAYER_WARNING,
							sound =
							"SpySociety/Actions/mainframe_gainCPU",
							icon = self.icon
...
function ResetProgramDefs()
	log:write("ResetProgramDefs()")
	util.tclear(mainframe_abilities)
	util.tmerge(mainframe_abilities, default_mainframe_abilities)
end
