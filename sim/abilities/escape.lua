...
local escape =
	{
		...
		canUseAbility = function( self, sim, unit )
			...
			if sim:hasTag( "exit_reqiuired_item" ) then
				...
				if agents <= agentsLeaving and not hasItem and not sim:getPC():getEscapedWithObjective() then
		...
		executeAbility = function( self, sim, abilityOwner )
			...
			if cell.exitID then
					...
					if c and c.exitID and unit:hasAbility( "escape" ) then
						...
						if player then
							...
							for i, child in ipairs(unit:getChildren())do
								if child:getTraits().isObjective then
									player:setEscapedWithObjective(true)
								end
							end							
						end						
					end
				...
				for i,unit in ipairs(units)do
					...
					if unit:getTraits().leavesAtEndOfMission then
					...
					if unit:getTraits().hostage then 
						if player then 
							sim:triggerEvent( "hostage_escaped", {unit=unit} )
						end
					end
