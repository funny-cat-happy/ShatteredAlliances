...
local compile_software =
	{
		name = STRINGS.ABILITIES.COMPILE_CODE,
        proxy = true,
		...
		onTooltip = function( self, hud, sim, abilityOwner, abilityUser, targetUnitID )
			...
			if targetUnit:getTraits().research_program then
				util.sformat(STRINGS.ABILITIES.COMPILE_CODE_DESC, STRINGS.ABILITIES.COMPILE_CODE, "gui/items/icon-action_hack-console.png" )
			end
			if reason then
		...
		isTarget = function( self, abilityOwner, unit, targetUnit )
			if not targetUnit:getTraits().research_program then
				return false
			end
		...
		executeAbility = function( self, sim, abilityOwner, unit, targetUnitID )
            ...
            --local triggerData = sim:triggerEvent(simdefs.TRG_COMPILE_START, { unit=targetUnit, sourceUnit=unit } )

            local triggerData = sim:triggerEvent( "compile_started", {unit=abilityOwner} )
            if not triggerData.abort then
           	...
           	targetUnit:getSounds().spot = "SpySociety/Objects/hightech_3"
