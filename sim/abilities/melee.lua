...
local melee = 
	{
		...
		executeAbility = function( self, sim, unit, userUnit, target )
			...
			if meleeDamage > 0 then
				...
				if sim:isVersion("0.17.12") and tazer:getTraits().armorPWRcost and targetUnit:getArmor() then
					local PWRcostPoints = returnPWRcostPoints(sim, userUnit, targetUnit)
					if PWRcostPoints > 0 then
						tazer:getPlayerOwner():addCPUs( -(tazer:getTraits().armorPWRcost * PWRcostPoints), sim, x1,y1)	
					end
				end
				...
				if lethal == true then
					...
					if unit:getTraits().convertKOtoPWR then
						...
						unit:getPlayerOwner():addCPUs(koTime, sim, x0,y0, 30 )						
				...
				if not sim:isVersion("0.17.12") and tazer:getTraits().armorPWRcost and targetUnit:getArmor() then
